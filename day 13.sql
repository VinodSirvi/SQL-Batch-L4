...
-- inner join 
select s.student_id , s.student_name , e.course_name , e.student_id from students as s 
...
-- group by
select student_name , count(e.enrollment_id) from students as s 
join enrollments as e 
on s.student_id = e.student_id
...
select s.student_id , s.student_name , sum(e.credits) , count(e.enrollment_id) from students as s 
left join enrollments as e 
on s.student_id = e.student_id 
...
left join enrollments as e 
on s.student_id = e.student_id 
group by student_id order by  s.student_name asc limit 5 offset 1;
