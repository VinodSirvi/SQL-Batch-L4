...
select month(sale_date) , sum(price_per_ticket) from ticket_sales group by month(sale_date);
...
select month(sale_date) , ticket_type ,  sum(price_per_ticket)  as month_revenue from ticket_sales  group by month(sale_date) , ticket_type  ;
...
select sale_id , event_id , price_per_ticket  from ticket_sales where price_per_ticket > (  select avg(price_per_ticket)  from ticket_sales );
...
select event_id , event_name , city from events where event_id in ( select event_id from ticket_sales where sale_date >= '2025-02-01' and sale_date <= '2025-02-28');
...
select month(sale_date) , sum(price_per_ticket) as total_revenue , sum(qty) as total_qty from ticket_sales group by month(sale_date) having total_revenue >= 8000;
...
select month(sale_date) as sale_month , avg(price_per_ticket)  as avg_sale_revenue from ticket_sales group by sale_month having avg_sale_revenue > 3000 ; 
...
select month(sale_date) as sale_month , ticket_type , sum(price_per_ticket) as total_revenue from ticket_sales group by sale_month having total_revenue >= 5000;
